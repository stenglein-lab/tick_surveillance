...
#metadata DF: only include CSID, Index, Sample_Type, Tick_Genus_species, Lifestage, county, state
meta_sub = meta[['Index', 'CSID', 'Sample_Type', 'Morphological_Ectoparasite_Genus_Species', 'Lifestage', 'County', 'State']]
...
meta_sub_seqs = pd.merge(seqs, meta_sub, on='Index').reindex(columns=['Index', 'CSID', 'abundance', 'primer_name', 'sequence_number', 'subject', 'percent_identity', 'Sample_Type', 'Morphological_Ectoparasite_Genus_Species', 'Lifestage', 'County', 'State', 'observed_sequence'])
